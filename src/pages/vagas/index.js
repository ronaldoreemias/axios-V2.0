...
function Vagas() {
    ...
    const aplicarFiltros = useCallback(() => {
        ...
        const tiposContratoSelecionados = Object.keys(filtros.tipoContrato).filter((key) => filtros.tipoContrato[key]);

        if (tiposContratoSelecionados.length > 0) {
            vagasFiltradas = vagasFiltradas.filter((vaga) => tiposContratoSelecionados.includes(vaga.tipoContrato));
        }

        // Filtro por área de atuação
        const areasSelecionadas = Object.keys(filtros.areaAtuacao).filter((key) => filtros.areaAtuacao[key]);

        if (areasSelecionadas.length > 0) {
            vagasFiltradas = vagasFiltradas.filter((vaga) => areasSelecionadas.includes(vaga.areaAtuacao));
        }

        // Filtro por modelo de trabalho
        const modelosSelecionados = Object.keys(filtros.modelo).filter((key) => filtros.modelo[key]);

        if (modelosSelecionados.length > 0) {
            vagasFiltradas = vagasFiltradas.filter((vaga) => modelosSelecionados.includes(vaga.modelo));
        }
    ...
    const renderBotaoCandidatar = (vaga) => {
        ...
        if (vaga.Email) {
            return (
                <button onClick={copiarEmail} className={style.botaoCandidatar}>
        ...
        return (
            <a href={link} target="_blank" rel="noopener noreferrer" className={style.botaoCandidatar}>
                {textoBotao}
            </a>
        );
    };
                                ...
                                {vagasFiltradas.map((vaga, index) => (
                                    <div key={index} className={style.cardVaga}>
                                        ...
                                        <div className={style.infoVaga}>
                                            <span className={style.localVaga}>📍 {vaga.Local}</span>
                                        </div>
